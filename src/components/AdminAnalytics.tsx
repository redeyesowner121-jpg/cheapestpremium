...
// Compound Analytics Chart Component
const CompoundAnalyticsChart: React.FC<{
  orders: Order[];
  users: User[];
  transactions: Transaction[];
  selectedPeriod: '7d' | '30d' | 'all';
}> = memo(({ orders, users, transactions, selectedPeriod }) => {
      ...
      return (
        <div className="bg-card border border-border rounded-lg p-3 shadow-lg">
          ...
          <div className="space-y-1 text-sm">
            <p className="text-success">💰 Deposits: ₹{data.deposits.toLocaleString()}</p>
            <p className="text-primary">📈 Sales: ₹{data.sales.toLocaleString()}</p>
            <p className="text-accent">👤 New Users: {data.newUsersCount} (₹{data.userGrowth} value)</p>
            <p className="text-secondary-foreground">📦 Products: {data.productsSoldCount} (₹{data.productSales} value)</p>
          </div>
        </div>
      );
    }
...
const AdminAnalytics: React.FC<AdminAnalyticsProps> = ({ orders, products, users = [], transactions = [] }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Compound Analytics Chart */}
      <CompoundAnalyticsChart 
        orders={orders} 
        users={users} 
