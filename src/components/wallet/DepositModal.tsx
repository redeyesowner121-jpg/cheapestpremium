...
import {
  ...
  Copy, ExternalLink, Loader2, Globe, MessageCircle, Search, CheckCircle
} from 'lucide-react';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm mx-auto rounded-3xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={depositTab} onValueChange={(v) => { setDepositTab(v as 'auto' | 'manual' | 'card'); setShowCardConfirm(false); }} className="mt-4">
              ...
              <div className="space-y-4">
                <div className="p-4 bg-primary/10 border border-primary/20 rounded-2xl text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    You will be redirected to the payment page. After payment, your deposit request will be automatically submitted for admin approval.
                  </p>
                </div>
                <Button
                  onClick={async () => {
                ...
                <Button variant="ghost" onClick={() => setShowCardConfirm(false)} className="w-full rounded-xl">
                  ← Go Back
                </Button>
              </div>
            )}
