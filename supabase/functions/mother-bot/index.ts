  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("mybot_stats_")) {
        ...
        await sendMsg(MOTHER_TOKEN, chatId, text, { reply_markup: { inline_keyboard: [[{ text: "◀️ Back", callback_data: `mybot_manage_${botId}`, style: "secondary" }]] } });
        return jsonOk();
      }
    ...
    if (update.message) {
      ...
      if (command === "/start") {
        ...
        if (channels.length > 0) {
          ...
          const buttons: any[][] = channels.map(ch => { const name = ch.startsWith("@") ? ch : `@${ch}`; return [{ text: `Join ${name}`, url: `https://t.me/${name.replace("@", "")}`, style: "primary" }]; });
            buttons.push([{ text: "✅ I've Joined - Verify", callback_data: "mother_verify_join", style: "success" }]);
