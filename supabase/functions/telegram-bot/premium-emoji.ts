...
const EMOJI_IDS: Record<string, string> = {
  ...
  airplane: "5283016693001470270",
};

// Global flag — set by initPremiumEmoji() at bot startup
...
export async function initPremiumEmoji(supabase: any): Promise<void> {
  try {
    const { data } = await supabase
      .from("app_settings")
      .select("value")
      .eq("key", "telegram_premium_emoji")
      .maybeSingle();
    premiumEnabled = data?.value === "true";
  } catch {
...
/**
 * Returns an HTML <tg-emoji> tag with fallback (if enabled),
 * otherwise just returns the fallback emoji.
 */
export function pe(name: string, fallback: string): string {
  if (!premiumEnabled) return fallback;
