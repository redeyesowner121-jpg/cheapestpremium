CREATE TABLE IF NOT EXISTS public.account_merge_audit (
  ...
  email text NOT NULL,
  merged_wallet_amount numeric NOT NULL DEFAULT 0,
  merged_total_deposit numeric NOT NULL DEFAULT 0,
  merged_rank_balance numeric NOT NULL DEFAULT 0,
  merged_pending_balance numeric NOT NULL DEFAULT 0,
  merged_total_savings numeric NOT NULL DEFAULT 0,
  moved_orders integer NOT NULL DEFAULT 0,
  moved_transactions integer NOT NULL DEFAULT 0,
  moved_notifications integer NOT NULL DEFAULT 0,
  moved_resale_links integer NOT NULL DEFAULT 0,
  moved_escrows integer NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (telegram_id, target_profile_id)
);

ALTER TABLE public.account_merge_audit ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "No direct account merge audit access" ON public.account_merge_audit;
CREATE POLICY "No direct account merge audit access"
ON public.account_merge_audit
FOR ALL
USING (false)
WITH CHECK (false);
...
BEGIN
  ...
  IF NOT FOUND AND v_target.id IS NULL THEN
